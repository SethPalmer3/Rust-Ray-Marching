...
mod test {
    ...
    fn test_surface_normal_x(){
        ...
        assert!((Vector3D::new(1.0, 0.0, 0.0) - s.get_surface_normal(&p, 0.0001)).length().abs() < 0.001);
    }
    #[test]
    fn test_surface_normal_y(){
        let s = Sphere::new(Point3D::new(0.0, 0.0, 0.0), 1.0, None);
        let p = Point3D::new(0.0, 1.0, 0.0);
        assert!((Vector3D::new(0.0, 1.0, 0.0) - s.get_surface_normal(&p, 0.0001)).length().abs() < 0.001);
    }
    #[test]
    fn test_surface_normal_z(){
        let s = Sphere::new(Point3D::new(0.0, 0.0, 0.0), 1.0, None);
        let p = Point3D::new(0.0, 0.0, 1.0);
        assert!((Vector3D::new(0.0, 0.0, 1.0) - s.get_surface_normal(&p, 0.0001)).length().abs() < 0.001);
    }
    #[test]
    fn test_surface_normal_xy(){
        let s = Sphere::new(Point3D::new(0.0, 0.0, 0.0), 1.0, None);
        let p = Point3D::new(1.0, 1.0, 0.0);
        assert!((Vector3D::new(1.0, 1.0, 0.0).get_norm() - s.get_surface_normal(&p, 0.0001)).length().abs() < 0.001);
    }
}
